...
async def breath(
    ...
    def _in_time_range(bucket):
        ...
        if _after and created < _after:
            return False
        if _before and created > _before:
            return False
        return True
    ...
    if importance_min >= 1:
        ...
        filtered = [b for b in filtered if _in_time_range(b)]
    ...
    if domain.strip().lower() == "feel":
        try:
            ...
            feels = [b for b in feels if _in_time_range(b)]
    ...
    # --- Time range filter ---
    # --- 时间范围过滤 ---
    matches = [b for b in matches if _in_time_range(b)]
...
async def hold(
    ...
    # --- Feel 模式：存为 feel 类型，自动打标让泡泡可被搜索 ---
    if feel:
        # Feel valence/arousal = model's own perspective
        feel_valence = valence if 0 <= valence <= 1 else 0.5
        feel_arousal = arousal if 0 <= arousal <= 1 else 0.3

        # Auto-tag feel bubbles so future selves can find them
        # 给感受泡泡打标，让以后的自己找得到
        feel_tags = []
        feel_domain = []
        feel_name = None
        ...
        try:
            ...
            feel_tags = feel_analysis.get("tags", [])
            feel_domain = feel_analysis.get("domain", [])
            feel_name = feel_analysis.get("suggested_name", "") or None
            feel_event_type = feel_analysis.get("event_type", "情感")
        except Exception as e:
            logger.warning(f"Feel auto-tagging failed, using defaults / 感受打标失败: {e}")

        # Merge user-supplied tags
        if extra_tags:
            feel_tags = list(dict.fromkeys(feel_tags + extra_tags))
...
async def api_config_update(request):
    ...
    if "dehydration" in body:
        ...
        dehydrator.model = dehy.get("model", "deepseek/deepseek-v4-flash")
