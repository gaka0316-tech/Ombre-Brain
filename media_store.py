...
把 MCP 调用携带的 Base64 数据或引用元数据持久化到 _media 目录。
对外暴露 MediaStore 和 MediaPersistenceError。
"""
...
_DEFAULT_MAX_MEDIA_BYTES = 25 * 1024 * 1024  # 25 MB
...
class MediaStore:
    ...
    def _stable_path(self, bucket_id: str, digest: str, suffix: str) -> Path:
        ...
        if self.media_dir not in target_dir.parents and target_dir != self.media_dir:
    ...
    def _persist_one(self, bucket_id: str, item: Any) -> dict[str, Any]:
        ...
        # If base64 data provided, persist the actual file
        if entry.get("data_base64"):
            ...
            # Reference-only mode: just normalize metadata
